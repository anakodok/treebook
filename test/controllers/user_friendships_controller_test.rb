...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:mul), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:mul), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
      ...
      should "display pending information on a pending friendship" do
        assert_select "div#user_friendship_#(@friendship1.id)" do
    ...
    context "when logged in" do
      ...
      context "successfully" do
        should "create two user friendship objects" do
          assert_difference "UserFriendship.count", 2 do
            post :create, user_friendship: { friend_id: users(:gaby) }
          end
        end
      end
      ...
      context "with a valid friend_id" do
        ...
        should "assign a friend object" do
          ...
          assert_equal users(:gaby), assigns(:friend)
        end

        should "assign a user_friendship object" do
          assert assigns(:user_friendship)
          assert_equal users(:mul), assigns(:user_friendship).user
          assert_equal users(:gaby), assigns(:user_friendship).friend
        end

        should "create a friendship" do
          assert users(:mul).pending_friends.include?(users(:gaby))
        end

        should "redirect to the profile page of the friend" do
          assert_response :redirect
          assert_redirected_to profile_path(users(:gaby))
        end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:mul), friend: @friend)
        create(:pending_user_friendship, user: @friend, friend: users(:mul))
        sign_in users(:mul)
      ...
      should "have a flash success message" do
        assert_equal "You are now friends with #{@user_friendship.friend.fullname}", flash[:success]
      end 
    end   
  end
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:mul))
        sign_in users(:mul)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:mul))
        create(:accepted_user_friendship, friend: users(:mul), user: @friend)
